...
class Carriers extends Model
{
    public static function list()
    {
        ...
        ->where('c.CUST_TYPE', 2)->where('c.BRANCH_ID', 'IHTVN1')
    ...
    public static function des($id)
    {
        try {
            $data = DB::table('CUSTOMER as c')
            ->join('CKICO_BRANCH as b', 'c.BRANCH_ID', '=', 'b.BRANCH_ID')
            ->where('c.CUST_TYPE', 2)->where('c.BRANCH_ID', 'IHTVN1')->where('c.CUST_NO', $id)
    ...
    public static function add($request)
    {
        try {
            ...
            DB::table(config('constants.CARRIERS_TABLE'))->insert(
                [
                    'CUST_TYPE' => 2,
    ...
    public static function edit($request)
    {
        try {
            date_default_timezone_set('Asia/Ho_Chi_Minh');
            DB::table(config('constants.CARRIERS_TABLE'))
